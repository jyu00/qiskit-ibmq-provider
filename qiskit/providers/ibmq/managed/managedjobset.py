...
class ManagedJobSet:
    ...
    def run(
        ...
        exp_index = 0
        logger.debug("Executor max worker is %s", executor._max_workers)  # type: ignore
        for i, experiments in enumerate(experiment_list):
            ...
            logger.debug("Calling ManagedJob.submit(). Pool threads are %s, pending jobs are %s",
                         str(len(executor._threads)),  # type: ignore
                         str(executor._work_queue.qsize()))  # type: ignore
