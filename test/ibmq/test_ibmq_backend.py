...
class TestIBMQBackend(IBMQTestCase):
    ...
    def test_backend_reservations(self):
        ...
        # Each tuple contains the start datetime, end datetime, and whether a
        # reservation should be found.
        sub_tests = [
            (before_start, after_end, True),
            (before_start, before_end, True),
            (after_start, before_end, True),
            (before_start, None, True),
            (None, after_end, True),
            (before_start, before_start, False),
            (after_end, after_end, False)
        ]

        for start_dt, end_dt, should_find in sub_tests:
            with self.subTest(start_dt=start_dt, end_dt=end_dt):
